...
let makeEnv formals instructions =
  ...
  let localVars =
    let buildLocalVars (varList : Symbol.t list) instruction =
      ...
      | Q.Instruction {label=_; op = Q.Gets {dst = Q.Id i; src=_}} ->
...
let concatStreams codestreams =
  List.fold_left CS.concat (List.hd codestreams) (List.tl codestreams)
(*done with doing this every time then below quick fix but not sure how to get call without u
...
let translateFunctionCall env callLabel opnds maybeLabel =
  ...
  in
  concatStreams [prologue; jal; epilogue]
...
let translateRHS env rhs maybeLabel reg =
  ...
  | Q.UnPrimOp  {op; opnd} ->
    let loadOpndCS = fIL [translateOperand operand1Reg env opnd maybeLabel] in
  ...
  |Q.BinPrimOp {op; opnds = {src1; src2}} ->
    let opnd1Inst = translateOperand operand1Reg env src1 maybeLabel in
    let opnd2Inst = translateOperand operand2Reg env src2 maybeLabel in
    let opndsCodestream = fIL [opnd1Inst; opnd2Inst] in
    let codeGenerator = lookupCodeGenerator op env in
    let opCodestream = codeGenerator [ Opnd.Reg reg; Opnd.Reg operand1Reg;
                                       Opnd.Reg operand2Reg] in
    concatStreams [opndsCodestream; opCodestream]
...
let translateOperation env opn maybeLabel procedure_label =
  ...
  | Q.Print rhs ->
    CS.concat
      ...
      (fIL [ toIn (None,
                   ...
                   Some "$v0 -> print int code for syscall")
  ...
  | Call {label; opnds} ->
    let prologue = callerPrologue opnds maybeLabel env in
    let jal = fIL [toIn (None, Opn.Jal (Opnd.Label label), None)] in
    let epilogue = callerEpilogue opnds in
    concatStreams [prologue; jal; epilogue]

(* NEED THIS AGAIN making this a funcation maybe take out *)
  ...
  | Q.Noop ->
    fIL [toIn (maybeLabel, Opn.Nop, None)]
  ...
  |Q.JmpZero {cond; dest} ->
    ...
    let beqz = fIL [toIn (maybeLabel, Opn.Beqz {rs; off18}, comment)] in
        CS.concat cond beqz
...
let translateProcedure (Q.Procedure {entry; formals; code}) =
  (* Make an environment (a mapping from identifiers to where they are stored on stack *)
  let (nLocals, env) = makeEnv formals code in
  (* Instructions to store frame pointer and bump stack pointer *)
  let prologue = calleePrologue entry nLocals in (* (nLocals + 1) in *)
  (* Translate each instruction in the procedure code and combine *)
  ...
  (* Epilogue prepares to return to the caller *)
